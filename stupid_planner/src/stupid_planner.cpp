...
StupidPlanner::StupidPlanner() {
  ...
  current_goal_pub_ =
      nh_.advertise<geometry_msgs::PoseStamped>("/goal_pos", 10);
}
...
void StupidPlanner::fcuInputGoalCallback(
  ...
  ROS_INFO("output p1 pos|vel: %.3f, %.3f, %.3f, %.3f| %.3f, %.3f, %.3f, %.3f",
    obst_free_path.point_1.position.x, obst_free_path.point_1.position.y, 
    obst_free_path.point_1.position.z, obst_free_path.point_1.yaw,
    obst_free_path.point_1.velocity.x, obst_free_path.point_1.velocity.y,
    obst_free_path.point_1.velocity.z, obst_free_path.point_1.yaw_rate);
  ROS_INFO("output p2 pos|vel: %.3f, %.3f, %.3f, %.3f| %.3f, %.3f, %.3f, %.3f",
    obst_free_path.point_2.position.x, obst_free_path.point_2.position.y, 
    obst_free_path.point_2.position.z, obst_free_path.point_2.yaw,
    obst_free_path.point_2.velocity.x, obst_free_path.point_2.velocity.y,
    obst_free_path.point_2.velocity.z, obst_free_path.point_2.yaw_rate);

  geometry_msgs::PoseStamped current;
  geometry_msgs::PoseStamped goal;
  current.header = msg.header;
  current.pose.position.x = msg.point_1.position.x;
  current.pose.position.y = msg.point_1.position.y;
  current.pose.position.z = msg.point_1.position.z;
  goal.header = msg.header;
  goal.pose.position.x = msg.point_2.position.x;
  goal.pose.position.y = msg.point_2.position.y;
  goal.pose.position.z = msg.point_2.position.z;
  current_pos_pub_.publish(current);
